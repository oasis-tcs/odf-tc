	...
	<xsl:template name="create-header">
		...
		<xsl:element name="head">
			...
			<xsl:if test="$debugEnabled">
				<xsl:message>CSS helper variable will be created....</xsl:message>
			</xsl:if>
			...
			<xsl:if test="$debugEnabled">
				<xsl:message>CSS variable ready, header will be created....</xsl:message>
			</xsl:if>
			...
			<xsl:if test="$debugEnabled">
				<xsl:message>CSS header creation finished!</xsl:message>
			</xsl:if>
		</xsl:element>
		...
		<xsl:element name="style">
			...
			<xsl:choose>
				<xsl:when test="/*/office:body/office:spreadsheet">
					<xsl:text>td, th { vertical-align:top; font-size:10pt;}
	</xsl:text>
				</xsl:when>
				<xsl:otherwise>
					<xsl:text>td, th { vertical-align:top; font-size:12pt;}
	</xsl:text>
				</xsl:otherwise>
			</xsl:choose>
	...
	<xsl:template name="write-mapped-CSS-styles">
		...
		<xsl:choose>
				...
				<xsl:for-each select="$globalData/all-styles/style[$styleNo]">
					<xsl:choose>
						<xsl:when test="final-properties != ''">
							...
							<xsl:text>.</xsl:text> <!--<xsl:value-of select="@style:family"/><xsl:text>:</xsl:text>-->
							<xsl:value-of select="translate(@style:name, '.,;: %()[]/\+', '_____________')" />
							<xsl:text> { </xsl:text>
							<xsl:value-of select="final-properties" />
							<xsl:text>}
	</xsl:text>
							<xsl:call-template name="write-mapped-CSS-styles">
								<xsl:with-param name="globalData" select="$globalData" />
			...
			<xsl:otherwise>
				...
				<xsl:comment> ODF styles with no properties representable as CSS </xsl:comment>
				<xsl:text></xsl:text>
				<xsl:value-of select="$emptyStyles" />
				<xsl:text>{ }
	</xsl:text>
			</xsl:otherwise>
		</xsl:choose>
