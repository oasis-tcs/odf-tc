...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"                 
    ...
    <!-- The specification -->
    <xsl:variable name="specification" select="/"/>
		...
		<xsl:if test="not(text:span[@text:style-name='Attribute_20_Value' or @text:style-name='Attribute_20_Value_20_Instance'])">
			<xsl:variable name="attr-name">
				<xsl:choose>
					...
					<xsl:otherwise>
						<xsl:value-of select="normalize-space(preceding::text:h[@text:outline-level='2'][1])"/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:variable>
        ...
        <xsl:copy>
            <xsl:choose>
                <xsl:when test="$convert-bookmarks-and-hyperlinks">
                    ...
                    <xsl:for-each select="text:bookmark[not(starts-with(@text:name,'__'))]|text:bookmark-start[not(starts-with(@text:name,'__'))]">
                        <xsl:sort select="@text:name" order="ascending"/>
                        <xsl:variable name="ref-name" select="@text:name"/>
                ...
                <xsl:copy>
                    ...
                    <xsl:choose>
                        <xsl:when test="preceding::text:h[@text:outline-level=$attributes-heading-level]=$attributes-heading">
                            <xsl:variable name="fp" select="preceding::text:h[@text:outline-level='1'][1]=$properties-heading"/>
                            <xsl:variable name="attr-name" select="normalize-space(preceding::text:h[@text:outline-level='2'][1])"/>
    ...
    <!-- **************************** -->
    <xsl:template match="text:h[@text:outline-level='3' and preceding::text:h[@text:outline-level='2'][1]=$datatypes-heading]">
        <xsl:copy>
            <xsl:apply-templates select="@*"/>
...
<!--            <xsl:copy>
    ...
    <xsl:template match="text:section">
        <xsl:message>Text sections are not supported: <xsl:value-of select="@text:name"/></xsl:message>
        <xsl:copy>
            <xsl:apply-templates select="@*|node()"/>
        </xsl:copy>
    </xsl:template>
                ...
                <xsl:when test="rng:ref[not(@condition)]|rng:data[not(@condition)]">
                    ...
                    <text:p text:style-name="Child_20_Element_20_List">
                        ...
                        <xsl:call-template name="print-datatype">
                            ...
                            <xsl:with-param name="attr-name" select="@name"/>
                        </xsl:call-template>
                        <xsl:text>.</xsl:text>
                    </text:p>
                </xsl:when>
    ...
    <xsl:template name="create-attr-parent-elem-list">
        ...
        <xsl:param name="element-list"/>
        <xsl:param name="fp"/>
        <xsl:variable name="parents" select="ancestor::rng:grammar/rng:element[((starts-with(@name,'style:') and contains(@name,'-properties'))=$fp) and ($element-list='' or contains($element-list,concat('_',$element-prefix,@name,'_'))) and rng:attribute[@name=$attr-name]]"/>
    ...
    <xsl:template name="create-attr-value-list">
        ...
        <xsl:choose>
            ...
            <xsl:when test="ancestor::rng:grammar/rng:element[((starts-with(@name,'style:') and contains(@name,'-properties'))=$fp) and (not($element-list) or contains($element-list,concat('_',$element-prefix,@name,'_'))) and count(rng:attribute[@name=$attr-name and not(contains(@condition,'rng:choice'))])>1]">
    ...
    <xsl:template match="rng:ref" mode="attr-value">
        ...
        <xsl:call-template name="new-line"/>
        <xsl:choose>
        	<xsl:when test="$specification/office:document-content/office:body/office:text/(text:list/text:list-item/text:p|text:h)[text:reference-mark-start[@text:name=concat($datatype-prefix,$name)]]">
    ...
    <xsl:template match="rng:data" mode="attr-value">
        ...
        <xsl:choose>
            ...
            <!--<xsl:when test="$specification/office:document-content/office:body/office:text/text:p[.=concat($datatype-prefix,$type)] and not(document($xref-schema-file)/rng:grammar/rng:define[@name=$type])">-->
        	<xsl:when test="$specification/office:document-content/office:body/office:text/(text:list/text:list-item/text:p|text:h)[text:reference-mark-start[@text:name=concat($datatype-prefix,$type)]]">
                <text:p text:style-name="Attribute_20_Value_20_List">
                    <xsl:text>The </xsl:text>
                    <text:span text:style-name="Attribute">
                        <xsl:value-of select="$attr-name"/>
            ...
            <xsl:otherwise>
                <xsl:message>*** Attribute <xsl:value-of select="$attr-name"/>: Unrecognized value element: <xsl:value-of select="name(.)"/> type="<xsl:value-of select="$type"/>"</xsl:message>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="rng:attribute[@name='text:reference-format']/rng:choice[not(count(*)=1)]" mode="attr-value">
        <xsl:param name="attr-name"/>
        <xsl:call-template name="new-line"/>
        <text:p text:style-name="Attribute_20_Value_20_List">
    ...
    <xsl:template match="rng:data[@type='string' or @type='NCName' or @type='anyURI']" mode="attr-value">
        <xsl:param name="attr-name"/>
        <text:p text:style-name="Attribute_20_Value_20_List">
            <xsl:text>The </xsl:text>
            <text:span text:style-name="Attribute">
    ...
    <xsl:template match="rng:data[@type='string' or @type='NCName' or @type='anyURI']" mode="attr-list-value">
        <xsl:param name="attr-name"/>
        <xsl:call-template name="print-datatype">
            <xsl:with-param name="name" select="@type"/>
            <xsl:with-param name="attr-name" select="$attr-name"/>
    ...
    <xsl:template match="*" mode="attr-list-value">
        <xsl:param name="attr-name"/>
        <xsl:message>*** Attribute <xsl:value-of select="$attr-name"/>: Unrecognized value element in list: <xsl:value-of select="name(.)"/> of type "<xsl:value-of select="@type"/>"</xsl:message>
    </xsl:template>
    ...
    <xsl:template match="rng:data[@type='string' or @type='NCName' or @type='anyURI']" mode="single-attr-list-value">
        <xsl:param name="attr-name"/>
        <xsl:text>is of type </xsl:text>
        <xsl:apply-templates select="." mode="individual-value">
            <xsl:with-param name="attr-name" select="$attr-name"/>
    ...
    <xsl:template match="rng:data[@type='string' or @type='NCName' or @type='anyURI']" mode="individual-value">
        <xsl:param name="attr-name"/>
        <xsl:text>a value of type </xsl:text>
        <xsl:call-template name="print-datatype">
            <xsl:with-param name="name" select="@type"/>
    ...
    <xsl:template match="*" mode="individual-value">
        <xsl:param name="attr-name"/>
        <xsl:message>*** Attribute <xsl:value-of select="$attr-name"/>: Unsupported individual value: <xsl:value-of select="name(.)"/> type "<xsl:value-of select="@type"/>"</xsl:message>
    </xsl:template>
